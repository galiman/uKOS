...
int32_t	stub_misc_getMode(uint32_t *mode) {
	...
	INTERRUPTION_OFF;
	switches |= ((GPIOC->IDR>>13) & 0x01);
	switches |= ((GPIOB->IDR<<1)  & 0x02);
	switches |= ((GPIOF->IDR>>8)  & 0x04);
	switches |= ((GPIOB->IDR>>2)  & 0x08);
	*mode = (uint32_t)(switches   & 0x0F);
...
int32_t	stub_misc_getExtension(uint8_t *byte, uint8_t offset) {
	static	volatile	uint8_t	*vTemp;

// Workaround FMC bug

	INTERRUPTION_OFF;
	vTemp = (volatile uint8_t *)(LOCBUSINT + offset);
	*byte = *vTemp;
	RETURN_INT_RESTORED(KMISCNOERR);
}
