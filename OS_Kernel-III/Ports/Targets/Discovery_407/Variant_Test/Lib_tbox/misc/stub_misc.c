...
void	stub_misc_init(void) {
	...
	GPIOD->ODR &= ~(1<<BLEDRED);
	GPIOD->ODR &= ~(1<<BLEDGREEN);
	GPIOD->ODR &= ~(1<<BLEDBLUE);
	GPIOD->ODR &= ~(1<<BLEDORANGE);
	GPIOC->ODR &= ~(1<<BLED0);
	GPIOC->ODR &= ~(1<<BLED1);
	GPIOE->ODR &= ~(1<<BLED2);
	GPIOC->ODR &= ~(1<<BLED3);
...
int32_t	stub_misc_getMode(uint32_t *mode) {
	...
	INTERRUPTION_OFF;
	*mode = (uint32_t)((GPIOD->IDR & 0x18)>>1 ) | (GPIOD->IDR & 0x03);
...
int32_t	stub_misc_onLed(uint8_t ledNb) {
	...
	switch (ledNb) {
		...
		case 8:  { GPIOD->ODR |= (1<<BLEDRED);    break; }
		case 9:  { GPIOD->ODR |= (1<<BLEDGREEN);  break; }
		case 10: { GPIOD->ODR |= (1<<BLEDBLUE);   break; }
		case 11: { GPIOD->ODR |= (1<<BLEDORANGE); break; }
		default: {
			break;
		}
	}
