...
static	const	boot_t	aFunction[] = {
							{ 0x00, "viewer",  KBD115200, KUSB0, 0, 0            },
							{ 0x01, "console", KBD115200, KUSB0, 2, argv_cnsUsb0 },
							{ 0x02, "sloader", KBD115200, KUSB0, 2, argv_sloader },
							{ 0x03, "sloader", KBD115200, KURT0, 2, argv_sloader }
						};

#define	KDEFCOMM	KUSB0
...
GLB_CONST_STRG(aStrHelp[]) =	"Start-up process\n"
								...
								"115200-bits/s, 8-bits, 2-stop-bits, no parity.\n\n"

								"   SW1\n"
								"    0   KDEF0, console (115200-bit/s).\n"
								"    1   KURT0, console (115200-bit/s).\n"
								"    2   KDEF0, sloader (115200-bit/s).\n"
								"    3   KURT0, sloader (115200-bit/s).\n\n";
...
// Set the default communication device (KSYST)
	...
	configureUSBx.oKernSync = (1<<BRXSEMA);
	usb0_configure(&configureUSBx);
...
// The last available function in the table is considered for the default communication manager
			...
			switch (aFunction[i].oCommManager) {
				case KUSB0: {                                                   usb0_configure(&configureUSBx); break; }
