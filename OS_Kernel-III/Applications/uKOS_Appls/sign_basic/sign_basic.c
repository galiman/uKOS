/*
; sign_basic.
; ===========

;------------------------------------------------------------------------
; Author:	Franzi Edo.	The 2006-06-28
; Modifs:
;
; SVN:
; $Author:: efr               $:  Author of last commit
; $Rev:: 186                  $:  Revision of last commit
; $Date:: 2017-08-17 15:22:27#$:  Date of last commit
;
; Project:	uKOS
; Goal:		Demo of a C application.
;			This application shows how to operate with the uKOS uKernel.
;
;			Launch 3 processes:
;
;			- P0: Repeat
;				  Wait 5000-ms
;				  Generate a signal "process 0" for the process P1
;				  Wait 1000-ms
;				  Generate a signal "process 0" for all the processes
;
;			- P1: Repeat
;				  Waiting for the signal from P0 (or broadcast) under timeout
;				  blink LED 0
;				  Display a string or a timeout error message
;
;			- P2: Repeat
;				  Waiting for the signal from P0 (or broadcast) under timeout
;				  blink LED 1
;				  Display a string or a timeout error message
;
;   (c) 1992-2017, Franzi Edo.
;   --------------------------
;                                              __ ______  _____
;   Franzi Edo.                         __  __/ //_/ __ \/ ___/
;   5-Route de Cheseaux                / / / / ,< / / / /\__ \
;   CH 1400 Cheseaux-Noréaz           / /_/ / /| / /_/ /___/ /
;                                     \__,_/_/ |_\____//____/
;   edo.franzi@ukos.ch
;
;   This program is free software: you can redistribute it and/or modify
;   it under the terms of the GNU Affero General Public License as published by
;   the Free Software Foundation, either version 3 of the License, or
;   (at your option) any later version.
;
;   This program is distributed in the hope that it will be useful,
;   but WITHOUT ANY WARRANTY; without even the implied warranty of
;   MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
;   GNU Affero General Public License for more details.
;
;   You should have received a copy of the GNU Affero General Public License
;   along with this program. If not, see <http://www.gnu.org/licenses/>.
;
;------------------------------------------------------------------------
*/

#include	<uKOS.h>

#define	KP0_TO_P1_SIGNAL_00000001	0x00000001
#define	KP0_TO_Pn_SIGNAL_01000000	0x01000000

// Module strings
// ==============

// ----------------------------------I------------I-----------------------------------------I--------------I

LOC_CONST_STRG(aStrApplication[]) =	"sign_basic   Example of how to use signals.            (c) EFr-2017";
LOC_CONST_STRG(aStrHelp[])        = "This is a romable C application\n"
									"===============================\n\n"

									"This user function module is a C written application.\n\n"

									"Input format:  sign_basic\n"
									"Output format: [result]\n\n";

LOC_CONST_STRG(aStrIden_0[]) 	  = "Process_Synchro 0";
LOC_CONST_STRG(aStrIden_1[]) 	  = "Process_User_1";
LOC_CONST_STRG(aStrIden_2[]) 	  = "Process_User_2";

// Module specifications (See the uKOS_module.h)
// =============================================

MODULE(UserAppl, KIDAPPLICATION, KAPPLICATIONNUM, _start, "1.0", ((1<<BSHOW) | (1<<BEXECONSOLE)));

/*
 * \brief aProcess 0
 *
 * - P0: - Repeat
 *			- Wait 5000-ms
 * 			- Generate a signal "process 0" for the process P1
 *			- Wait 1000-ms
 * 			- Generate a signal "process 0" for all the processes
 *
 */
static	void	aProcess_0(const void *argument) {
	volatile	sign_t		*signal;
	volatile	proc_t 		*process;

	if (kern_createSignal("Process 0", &signal)      != KKERNNOERR) { exit(EXIT_FAILURE);     }
	while (kern_getProcessById(aStrIden_1, &process) != KKERNNOERR) { kern_suspendProcess(1); }

	while (TRUE) {
		kern_suspendProcess(5000);
		kern_signalSignal(signal, KP0_TO_P1_SIGNAL_00000001, process, KSIGNSWTC);
		kern_suspendProcess(1000);
		kern_signalSignal(signal, KP0_TO_Pn_SIGNAL_01000000, NULL, KSIGNSWTC);
	}
}

/*
 * \brief aProcess 1
 *
 * - P1: - Waiting for the signal "process 0" (or broadcast) under timeout
 *  		- Blink LED 0
 * 			- Display a string or a timeout error message
 *
 */
static	void	aProcess_1(const void *argument) {
	int32_t		status;
	uint32_t	signalProcess;

// Get the signal

	while (TRUE) {
		kern_suspendProcess(200);
		misc_offLed(0);

		signalProcess = KP0_TO_P1_SIGNAL_00000001 | KP0_TO_Pn_SIGNAL_01000000;
		status = kern_waitSignal(&signalProcess, 10000);
		misc_onLed(0);

		(status == KKERNTIMEO) ? iotx_printf(KSYST, "P1 - Timeout error! %08X\n", signalProcess) : iotx_printf(KSYST, "P1 - Signal %08X\n", signalProcess);
	}
}

/*
 * \brief aProcess 2
 *
 * - P2: - Waiting for the signal "process 0" (or broadcast) under timeout
 *  		- Blink LED 0
 * 			- Display a string or a timeout error message
 *
 */
static	void	aProcess_2(const void *argument) {
	int32_t		status;
	uint32_t	signalProcess;

// Get the signal

	while (TRUE) {
		kern_suspendProcess(200);
		misc_offLed(1);

		signalProcess = KP0_TO_Pn_SIGNAL_01000000;
		status = kern_waitSignal(&signalProcess, 10000);
		misc_onLed(1);

		(status == KKERNTIMEO) ? iotx_printf(KSYST, "P2 - Timeout error! %08X\n", signalProcess) : iotx_printf(KSYST, "P2 - Signal %08X\n", signalProcess);
	}
}

/*
 * \brief main
 *
 * - Initialize the used libraries
 * - Launch all the processes
 * - Kill the "main". At this moment only the launched processes are executed
 *
 */
int		main(void) {
	volatile	proc_t 	*process_0, *process_1, *process_2;

// ------------------------------------I-----------------------------------------I--------------I

	LOC_CONST_STRG(aStrText_0[]) =    "Process Synchro 0.                        (c) EFr-2017";
	LOC_CONST_STRG(aStrText_1[]) =    "Process User 1.                           (c) EFr-2017";
	LOC_CONST_STRG(aStrText_2[]) =    "Process User 2.                           (c) EFr-2017";

// Specifications for the processes

	PROC_SUPV(0, vSpecification_0, aStrText_0, KSZSTACKMIN, aProcess_0, aStrIden_0, KDEF0, (KMEDIUMPRIORITY+1), 0, KPROCNORMAL);
	PROC_SUPV(1, vSpecification_1, aStrText_1, KSZSTACKMIN, aProcess_1, aStrIden_1, KDEF0, (KMEDIUMPRIORITY+1), 0, KPROCNORMAL);
	PROC_SUPV(2, vSpecification_2, aStrText_2, KSZSTACKMIN, aProcess_2, aStrIden_2, KDEF0, (KMEDIUMPRIORITY+1), 0, KPROCNORMAL);

	if (kern_createProcess(&vSpecification_0, NULL, &process_0) != KKERNNOERR) { exit(EXIT_FAILURE); }
	if (kern_createProcess(&vSpecification_1, NULL, &process_1) != KKERNNOERR) { exit(EXIT_FAILURE); }
	if (kern_createProcess(&vSpecification_2, NULL, &process_2) != KKERNNOERR) { exit(EXIT_FAILURE); }
	return (EXIT_SUCCESS_OS);
}

