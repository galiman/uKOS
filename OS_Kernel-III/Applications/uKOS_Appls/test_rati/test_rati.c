...
;
;			- P0: Every 10000-ms
;				  Compute the occupation time of the running tasks (cpt_Px / SIM(cpt_P0..2))
;				  Display the occupation time of the processes P1, P2 and P3
;
...
LOC_CONST_STRG(aStrApplication[]) =	"test_rati    uKernel test of the cpu occupation time.  (c) EFr-2017";
LOC_CONST_STRG(aStrHelp[])        = "This is a romable C application\n"
									"===============================\n\n"

									"This user function module is a C written application.\n\n"

									"Input format:  test_rati\n"
...
static	void	aProcess_0(const void *argument) {
	uint32_t	summ, delta_1, delta_2, delta_3 = 1;

	while (TRUE) {
		kern_suspendProcess(1000);
		delta_1 = vCpt_1 - vCpt_3;
		delta_2 = vCpt_2 - vCpt_3;

		summ = delta_1 + delta_2 + delta_3;
...
static	void	aProcess_1(const void *argument) {
	...
	while (TRUE) { vCpt_1++; kern_switchFast(); }
}
...
static	void	aProcess_2(const void *argument) {
	...
	while (TRUE) { vCpt_2++; kern_switchFast(); }
}
...
static	void	aProcess_3(const void *argument) {
	...
	while (TRUE) { vCpt_3++; kern_switchFast(); }
}
