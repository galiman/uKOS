...
LOC_CONST_STRG(aStrApplication[]) =	"urt1         urt1 manager.                             (c) EFr-2017";
LOC_CONST_STRG(aStrHelp[])        = "urt1 manager\n"
									"============\n\n"
...
extern	int32_t	stub_urt1_configure(const cnfUrtx_t *configure);
...
extern	int32_t	stub_urt1_flush(void);

// Module specifications (See the uKOS_module.h)
// =============================================

MODULE(Urt1, KIDCOMM, KURT1NUM, NULL, "2.0", (1<<BSHOW));
 ...
 *
 */
int32_t	urt1_reserve(uint8_t mode) {
	...
	switch (mode) {
		case KDEVREAD: {
			if (vReadReserved)  			     {												RETURN_INT_RESTORED(KURT1CHBSY); }
			else 		       				     { vReadReserved = TRUE;						RETURN_INT_RESTORED(KURT1NOERR); }
			break;
		}
		case KDEVWRITE: {
			if (vWriteReserved) 				 { 											    RETURN_INT_RESTORED(KURT1CHBSY); }
			else 		       				     { vWriteReserved = TRUE;						RETURN_INT_RESTORED(KURT1NOERR); }
			break;
		}
		case KDEVALL: {
			if (vReadReserved || vWriteReserved) {                                              RETURN_INT_RESTORED(KURT1CHBSY); }
			else 		       				     { vReadReserved = TRUE; vWriteReserved = TRUE;	RETURN_INT_RESTORED(KURT1NOERR); }
			break;
		}
		default: {
			break;
	...
	RETURN_INT_RESTORED(KURT1NOERR);
}
 ...
 *
 */
int32_t	urt1_release(uint8_t mode) {

	_init();

	INTERRUPTION_OFF;
	switch (mode) {
		case KDEVREAD:  { vReadReserved = FALSE;                         RETURN_INT_RESTORED(KURT1NOERR); break; }
		case KDEVWRITE: {                        vWriteReserved = FALSE; RETURN_INT_RESTORED(KURT1NOERR); break; }
		case KDEVALL:   { vReadReserved = FALSE; vWriteReserved = FALSE; RETURN_INT_RESTORED(KURT1NOERR); break; }
		default: {
			break;
		}
	}
 ...
 *
 */
int32_t	urt1_configure(const cnfUrtx_t *configure) {

	_init();
	return (stub_urt1_configure(configure));
}
 ...
 *
 */
int32_t	urt1_write(const uint8_t *buffer, uint32_t size) {

	_init();
	return (stub_urt1_write(buffer, size));
}
 ...
 *
 */
int32_t	urt1_read(uint8_t *buffer, uint32_t *size) {

	_init();
	return (stub_urt1_read(buffer, size));
}
 ...
 *
 */
int32_t	urt1_getIdSemaphore(uint8_t semaphore, char_t **identifier) {

	_init();
	if (semaphore == KSEMARX) { *identifier = KSEMAPHORE_URT1RX; return (KURT1NOERR); }
	if (semaphore == KSEMATX) { *identifier = KSEMAPHORE_URT1TX; return (KURT1NOERR); }
	return (KURT1SENOE);
}
 ...
 *
 */
int32_t	urt1_flush(void) {

	_init();
	return (stub_urt1_flush());
}
