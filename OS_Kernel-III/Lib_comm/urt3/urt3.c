...
LOC_CONST_STRG(aStrApplication[]) =	"urt3         urt3 manager.                             (c) EFr-2017";
LOC_CONST_STRG(aStrHelp[])        = "urt3 manager\n"
									"============\n\n"
...
extern	int32_t	stub_urt3_configure(const cnfUrtx_t *configure);
...
MODULE(Urt3, KIDCOMM, KURT3NUM, NULL, "2.0", (1<<BSHOW));
 ...
 *
 */
int32_t	urt3_reserve(uint8_t mode) {
	...
	switch (mode) {
		case KDEVREAD: {
			if (vReadReserved)  			     {												RETURN_INT_RESTORED(KURT3CHBSY); }
			else 		       				     { vReadReserved = TRUE;						RETURN_INT_RESTORED(KURT3NOERR); }
			break;
		}
		case KDEVWRITE: {
			if (vWriteReserved) 				 { 											    RETURN_INT_RESTORED(KURT3CHBSY); }
			else 		       				     { vWriteReserved = TRUE;						RETURN_INT_RESTORED(KURT3NOERR); }
			break;
		}
		case KDEVALL: {
			if (vReadReserved || vWriteReserved) {                                              RETURN_INT_RESTORED(KURT3CHBSY); }
			else 		       				     { vReadReserved = TRUE; vWriteReserved = TRUE;	RETURN_INT_RESTORED(KURT3NOERR); }
			break;
		}
		default: {
			break;
	...
	RETURN_INT_RESTORED(KURT3NOERR);
}
 ...
 *
 */
int32_t	urt3_release(uint8_t mode) {

	_init();

	INTERRUPTION_OFF;
	switch (mode) {
		case KDEVREAD:  { vReadReserved = FALSE;                         RETURN_INT_RESTORED(KURT3NOERR); break; }
		case KDEVWRITE: {                        vWriteReserved = FALSE; RETURN_INT_RESTORED(KURT3NOERR); break; }
		case KDEVALL:   { vReadReserved = FALSE; vWriteReserved = FALSE; RETURN_INT_RESTORED(KURT3NOERR); break; }
		default: {
			break;
		}
	}
 ...
 *
 */
int32_t	urt3_configure(const cnfUrtx_t *configure) {

	_init();
	return (stub_urt3_configure(configure));
}
 ...
 *
 */
int32_t	urt3_write(const uint8_t *buffer, uint32_t size) {

	_init();
	return (stub_urt3_write(buffer, size));
}
 ...
 *
 */
int32_t	urt3_read(uint8_t *buffer, uint32_t *size) {

	_init();
	return (stub_urt3_read(buffer, size));
}
 ...
 *
 */
int32_t	urt3_getIdSemaphore(uint8_t semaphore, char_t **identifier) {

	_init();
	if (semaphore == KSEMARX) { *identifier = KSEMAPHORE_URT3RX; return (KURT3NOERR); }
	if (semaphore == KSEMATX) { *identifier = KSEMAPHORE_URT3TX; return (KURT3NOERR); }
	return (KURT3SENOE);
}
 ...
 *
 */
int32_t	urt3_flush(void) {

	_init();
	return (stub_urt3_flush());
}
