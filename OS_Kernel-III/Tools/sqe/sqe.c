...
;
; Project:	uKOS
; Goal:		Interface for Sysquake Embedded (Calerga Sarl, www.calerga.com)
;
...
#define	KSIZEBUFSQE	90000

static				uint32_t	vSize = KSIZEBUFSQE;
...
LOC_CONST_STRG(aStrApplication[]) =	"sqe          Sysquake Embedded for uKOS.               (c) EFr-2017";
LOC_CONST_STRG(aStrHelp[])        = "Sysquake Embedded for uKOS\n"
									"==========================\n\n"

									"This tool allows to use Sysquake Embedded\n"
									" (Calerga Sarl, www.calerga.com) in the uKOS workspace.\n\n"

									"Input format:  sqe commManager, {urt0, ... }\n"
							        "                   memoryLocation, {-internal | -external}\n"
							        "                   memorySize\n"
									"Output format: sqe results\n\n";

LOC_CONST_STRG(aStrIden[])        = "Process_sqe";
LOC_CONST_STRG(aStrText[])        = "Process Sysquake Embedded (Calerga Sarl)  (c) EFr-2017";
...
MODULE(Sqe, KIDTOOL, KSQENUM, prgm, "1.3", ((1<<BSHOW) | (1<<BEXECONSOLE)));
...
// Try to reserve the SQE memory segment
	...
	switch (error) {
		case KERRNOT: {
			iotx_printf(KSYST, "Sysquake Embedded launcher.\n");
			iotx_printf(KSYST, __DATE__"  "__TIME__" (c) EFr-2017\n\n");

			PROC_SUPV_STACKMALLOC(0, KINTERNAL, vSpecification_0, aStrText, KSZSTACKSQE, _process, aStrIden, KDEF0, (KHIGHPRIORITY+1), 0, KPROCNORMAL);
		...
		case KERRLOK: { iotx_printf(KSYST, "SQE session is running.\n\n");             status = EXIT_FAILURE;    vLock = FALSE; break; }
