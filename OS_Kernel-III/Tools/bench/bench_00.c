...
;			_fill		552				28				90				26
;			_projX		831				46				142				44
;			_projY		1129			46				142				43
;			_histo		1225			316				314				123
;
 ...
 *
 */
bool_t		bench_00(void) {
	uint8_t		*array;
	uint32_t	*x, *y, *h, time[4];

	array = (uint8_t  *)syos_malloc(KINTERNAL, (KNBX*KNBY));             if (array == NULL) {                                 				return (FALSE); }
	x     = (uint32_t *)syos_malloc(KINTERNAL, (KNBX*sizeof(uint32_t))); if (x == NULL)     { syos_free(array);               				return (FALSE); }
	y     = (uint32_t *)syos_malloc(KINTERNAL, (KNBY*sizeof(uint32_t))); if (y == NULL)     { syos_free(array); syos_free(x); 				return (FALSE); }
	h     = (uint32_t *)syos_malloc(KINTERNAL, (256*sizeof(uint32_t)));  if (h == NULL)     { syos_free(array); syos_free(x); syos_free(y); return (FALSE); }
...
// ---------------------|---------|--------------------------------------------|-----------------|
//
	iotx_printf(KSYST, "Bench 00: Fill a small 2D array (50 x 50) elements in\n"
