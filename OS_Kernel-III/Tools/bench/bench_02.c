...
;
; Project:	uKOS
; Goal:		Bench 02: 	Searching for the min / max of a small square array (1000 uint32_t) elements.
 ...
 *
 */
bool_t		bench_02(void) {
	uint32_t	i, *array, value, time[1], min, max;

	array = (uint32_t *)syos_malloc(KINTERNAL, (KNBELEMENTS*sizeof(uint32_t))); if (array == NULL) return (FALSE);
...
// ---------------------|---------|--------------------------------------------|-----------------|
//
	iotx_printf(KSYST, "Bench 02: Fill a small 1D array (1000) elements in\n"
